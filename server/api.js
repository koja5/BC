            ...
            conn.query(
                ...
                function(err, rows, fields) {
                    ...
                    if (rows.length !== 0) {
                        ...
                        conn.query(
                            "SELECT * FROM users WHERE email=?", [req.body.data.email],
                            function(err, rows, fields) {
                                if (err) {
                                    console.error("SQL error:", err);
                                    res.json({
...
router.post("/updateMember", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        delete req.body.bic;
        conn.query(
            "UPDATE users SET ? where id = '" + req.body.id + "'", [req.body],
            function(err, rows) {
                conn.release();
                if (err) {
                    res.send(err);
...
router.get("/getExperience/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from experience where id_user = ? order by toDate asc", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.get("/getEducation/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from education where id_user = ? order by toDate asc", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
...
router.get("/getAdditionalInfo/:id", function(req, res, next) {
    connection.getConnection(function(err, conn) {
        ...
        var id = req.params.id;
        conn.query(
            "SELECT * from additionalInfo where id_user = ?", [id],
            function(err, rows) {
                conn.release();
                if (!err) {
                    res.json(rows);
